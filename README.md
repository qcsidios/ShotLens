...
- 固定把英文翻译为简体中文；OCR 保留中英文、数字和标点的完整上下文，模型只翻译英文并保留已有中文
...
ShotLens 默认继续使用 macOS Vision 在本机 OCR：原图准确识别作为主结果，轻量灰度、对比度和锐化增强图只补回浅色漏字，截图像素仍不上传。OCR 保留完整中英文行、数字和标点，排除纯图标、纯符号及触碰框选边缘的不完整文字；随后只按行距、对齐和视觉样式做简单几何分组，不猜测页面属于菜单、文章或卡片。这样既保留整段语境，又继续使用 Vision 返回的坐标完成原位覆盖。云端 OCR 只有在提供稳定逐块坐标契约，并且隐私、网络成本和端到端延迟实测优于本地 Vision 时才重新评估。
