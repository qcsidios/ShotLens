...
- 译文按独立 OCR 英文片段的像素坐标锚定；正常使用周边背景像素局部重建，重建不可用时只在该片段的原始范围内使用采样背景兜底
...
翻译输入使用带稳定编号的 JSON 对象，每个英文片段始终对应自己的 OCR 坐标。默认 SiliconFlow 接口启用 JSON 输出模式；使用 Xiaomi MiMo 官方接口时还会关闭思考输出、限制回复长度并使用官方 `api-key` 请求头，避免翻译前生成无用推理。标准编号对象优先解析，数组、编号列表、代码块、常见箭头分隔、解释前缀、SSE 边缘残片、控制标记和已确认的近似 JSON 会在本地清洗，不会追加网络修复请求。常规框选保持一次请求；超大选区按容量分批，但每批只请求一次。模型漏译或误答单个片段时保留其他可靠译文，失败片段继续显示原文。常见 UI 词会逐项在本地完成，只把未知文本发送给 API；同一 App 会话内完全相同的完整成功翻译最多缓存 128 批，退出后自动清空且不写入磁盘。产品名、型号和标识符允许合理保留英文。

浮框里的“重新翻译”会直接复用当前 OCR 英文片段，只重新调用翻译 API；OCR 尚未成功时才重新识别当前框选截图。整个过程不会再次截取屏幕，也不会捕获已经显示的译文或要求用户重新框选。

ShotLens 默认继续使用 macOS Vision 在本机 OCR：先对轻量灰度、对比度和锐化增强图执行准确识别，没有得到可用英文时再自动回到原图识别；只启用英文识别，减少中文、图标和符号造成的误判与语言检测开销，截图像素仍不上传。只有包含完整英文词且未触碰框选边缘的独立片段才进入翻译，不再进行菜单、文章或段落类型猜测，也不跨行合并坐标。Vision 能直接返回原位覆盖所需的逐块坐标，避免第二次网络推理。云端 OCR 只有在提供稳定逐块坐标契约，并且隐私、网络成本和端到端延迟实测优于本地 Vision 时才重新评估。
