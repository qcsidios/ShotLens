...
3. 如需使用自己的额度，可点击“自备 API”填写 API 地址、密钥和模型。
4. 点击“测试”确认当前配置可用。
5. 授予屏幕录制权限。
...
ShotLens 使用兼容 OpenAI 的聊天补全接口。默认限免模式内置 SiliconFlow 地址、公共限免 Key 和 `tencent/Hunyuan-MT-7B` 模型，不在控制台显示这些信息，也不允许切换到其他模型；如需自定义配置，请点击“自备 API”。
